...
class TestForceColorMatch:
    """Test the force_color_match parameter in _match_filaments_to_slots."""

    @pytest.fixture
    def scheduler(self):
        return PrintScheduler()

    def test_force_color_match_false_allows_type_only(self, scheduler):
        """Without force_color_match, type-only matches are used as fallback."""
        required = [{"slot_id": 1, "type": "PLA", "color": "#FF0000"}]
        loaded = [
            {"type": "PLA", "color": "#0000FF", "global_tray_id": 5},  # Type match, wrong color
        ]
        result = scheduler._match_filaments_to_slots(required, loaded, force_color_match=False)
        assert result == [5]

    def test_force_color_match_true_blocks_type_only(self, scheduler):
        """With force_color_match=True, type-only matches are NOT used."""
        required = [{"slot_id": 1, "type": "PLA", "color": "#FF0000"}]
        loaded = [
            {"type": "PLA", "color": "#0000FF", "global_tray_id": 5},  # Type match, wrong color
        ]
        result = scheduler._match_filaments_to_slots(required, loaded, force_color_match=True)
        assert result == [-1]

    def test_force_color_match_true_allows_exact_match(self, scheduler):
        """With force_color_match=True, exact color matches are still used."""
        required = [{"slot_id": 1, "type": "PLA", "color": "#FF0000"}]
        loaded = [
            {"type": "PLA", "color": "#FF0000", "global_tray_id": 3},
        ]
        result = scheduler._match_filaments_to_slots(required, loaded, force_color_match=True)
        assert result == [3]

    def test_force_color_match_true_allows_similar_match(self, scheduler):
        """With force_color_match=True, similar color matches are still used."""
        required = [{"slot_id": 1, "type": "PLA", "color": "#FF5500"}]
        loaded = [
            {"type": "PLA", "color": "#FF5510", "global_tray_id": 2},  # Similar color
        ]
        result = scheduler._match_filaments_to_slots(required, loaded, force_color_match=True)
        assert result == [2]

    def test_force_color_match_omitted_uses_type_only_fallback(self, scheduler):
        """When force_color_match is not passed, type-only fallback is used (backward-compatible)."""
        required = [{"slot_id": 1, "type": "PLA", "color": "#FF0000"}]
        loaded = [
            {"type": "PLA", "color": "#0000FF", "global_tray_id": 5},
        ]
        result = scheduler._match_filaments_to_slots(required, loaded)
        # Default should still use type_only (backward compatible)
        assert result == [5]

    def test_force_color_match_multi_filament_partial_no_match(self, scheduler):
        """With force_color_match=True, unmatched slots get -1 even if type matches."""
        required = [
            {"slot_id": 1, "type": "PLA", "color": "#FF0000"},
            {"slot_id": 2, "type": "PETG", "color": "#00FF00"},
        ]
        loaded = [
            {"type": "PLA", "color": "#FF0000", "global_tray_id": 0},   # Exact match
            {"type": "PETG", "color": "#0000FF", "global_tray_id": 1},  # Type match, wrong color
        ]
        result = scheduler._match_filaments_to_slots(required, loaded, force_color_match=True)
        # Slot 1 gets match (exact), slot 2 gets -1 (type-only not allowed)
        assert result == [0, -1]

    def test_force_color_match_with_tray_info_idx(self, scheduler):
        """With force_color_match=True, tray_info_idx unique match still works."""
        required = [{"slot_id": 1, "type": "PLA", "color": "#FF0000", "tray_info_idx": "GFA00"}]
        loaded = [
            {"type": "PLA", "color": "#0000FF", "global_tray_id": 5, "tray_info_idx": "GFA00"},
        ]
        # Unique tray_info_idx takes priority — even over color mismatch
        result = scheduler._match_filaments_to_slots(required, loaded, force_color_match=True)
        assert result == [5]
